...
def heuristic_advanced(board, player):
    ...
    """
    empty_player = 0
    for i in board.pockets[player]:
        if i == 0:
            empty_player += 1
    empty_opponent = 0
    for i in board.pockets[get_opponent(player)]:
        if i == 0:
            empty_opponent += 1

    empty_lead = empty_player - empty_opponent
    mancalas_lead = board.mancalas[player] - board.mancalas[get_opponent(player)]
    return empty_lead + mancalas_lead
